              ...
              <div className="relative group">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={assetDepreciation} onChange={(e) => handleInputChange('assetDepreciation', e.target.value, setAssetDepreciation)}
                    className={`w-full bg-transparent border-0 border-b-2 pl-8 py-2 text-xl md:text-2xl font-bold transition-colors focus:outline-none ${showValidation && validations.assetDepreciation?.type === 'warning' ? 'border-tertiary text-tertiary focus:border-tertiary' : 'border-outline-variant text-on-surface focus:border-primary'}`} />
                </div>
    ...
    <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1" data-tour="calculator">
          {/* Tabs */}
          <div className="flex flex-wrap gap-1.5 lg:gap-2 mb-8 bg-surface-container-low p-1.5 rounded-2xl">
            {TAX_TABS.map(tab => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setResult(null); }}
                className={`px-3 lg:px-6 py-2 lg:py-2.5 rounded-xl font-semibold transition-all text-xs lg:text-sm whitespace-nowrap ${activeTab === tab.id ? 'bg-surface-container-lowest text-on-surface shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          <div className="bg-surface-container-lowest p-4 md:p-8 rounded-3xl shadow-taxfyp border border-outline-variant/15" data-tour="smart-validation">
            ...
            {showUploader && activeTab === 'CIT' && (
              <div className="mb-8 p-6 bg-surface-container-low rounded-2xl border border-outline-variant/20">
                <BankStatementUploader onTransactionsExtracted={handleTransactionsExtracted} />
              </div>
            )}
            ...
            <div className="mt-12 flex flex-col sm:flex-row justify-end gap-3">
              ...
              }} className="bg-surface-container-low text-on-surface px-6 py-3 rounded-xl font-semibold hover:bg-surface-container-high transition-colors">
                Clear
              </button>
              <button onClick={calculateTax} disabled={isCalculating}
                className="premium-gradient text-white px-8 py-3 rounded-xl font-semibold shadow-taxfyp hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="w-full lg:w-96">
          <div className="lg:sticky lg:top-28 space-y-6">
            ...
            <div className="bg-surface-container-low p-6 rounded-3xl border border-outline-variant/20">
              ...
              <button onClick={() => window.dispatchEvent(new CustomEvent('navigateTo', { detail: 'scenario' }))}
